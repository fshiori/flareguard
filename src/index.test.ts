...
describe("app", () => {
  ...
  it("forwards Workers script deployment creates from Wrangler raw account paths when the script grant matches", async () => {
    const secretHash = await hashProxySecret("secret");
    const fetchMock = vi.fn(async () => Response.json({ success: true }));
    vi.stubGlobal("fetch", fetchMock);
    ...
    const res = await app.request("/accounts/acct_1/workers/scripts/script-a/deployments", {
      ...
      body: JSON.stringify({ strategy: "percentage", versions: [{ version_id: "version_1", percentage: 100 }] })
    }, createEnv({
      keyRow: {
        id: "fgk_123",
        account_id: "acct_1",
  ...
  it("forwards Workers script subdomain reads from Wrangler raw account paths when the script grant matches", async () => {
    const secretHash = await hashProxySecret("secret");
    const fetchMock = vi.fn(async () => Response.json({ success: true }));
    vi.stubGlobal("fetch", fetchMock);

    const res = await app.request("/accounts/acct_1/workers/scripts/script-a/subdomain", {
      method: "GET",
      headers: { Authorization: "Bearer fgk_123.secret" }
    }, createEnv({
      keyRow: {
  ...
  it("forwards Workers script subdomain updates from Wrangler raw account paths when the script grant matches", async () => {
    const secretHash = await hashProxySecret("secret");
    const fetchMock = vi.fn(async () => Response.json({ success: true }));
    vi.stubGlobal("fetch", fetchMock);
